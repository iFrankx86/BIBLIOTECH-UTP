...
const LoanModal = ({ show, onHide, loan }: LoanModalProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    returnDate: loan?.returnDate ? new Date(loan.returnDate).toISOString().split('T')[0] : '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const effectiveStatus = formData.status as Loan['status'];
    const effectiveReturnDate = effectiveStatus === 'returned'
    ...
    if (loan) {
      ...
      if (loan.reservationId) updatedLoan.reservationId = loan.reservationId;
      if (effectiveReturnDate) updatedLoan.returnDate = effectiveReturnDate;
      ...
      if (effectiveReturnDate) newLoan.returnDate = effectiveReturnDate;
      await addLoan(newLoan);
    }

    onHide();
    setFormData({ bookId: '', memberId: '', loanDays: 14, status: 'active', returnDate: '' });
  };
